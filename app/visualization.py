...
def visualize_embedding(bert_embedding, masks, epoch, batch_no, kl_coefficient, type="bert") -> None:
    saved_path = os.path.join("plots", type, f"coef-{str(kl_coefficient)}", f"epoch-{str(epoch + 1)}")
...
def visualize_latent_vector_space(latent_vector_space, epoch, batch_count, kl_coefficient, type="latent_vector_space") -> None:
    saved_path = os.path.join("plots", type, f"coef-{str(kl_coefficient)}", f"epoch-{str(epoch + 1)}")
    os.makedirs(saved_path, exist_ok=True)
    dim_reducer = TSNE(n_components=2,  perplexity=10.0)
