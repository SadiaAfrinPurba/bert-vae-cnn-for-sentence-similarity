...
def start_training(config: Config):
    ...
    for kl_coefficent in [0, 0.2, 0.5]:
        print(f"=======Working for KL {kl_coefficent}========================")
        for epoch in range(config.num_epochs):
            ...
            for input_ids, attention_mask in data_loader:
                ...
                with torch.no_grad():
                    ...
                    visualize_embedding(bert_embedding=bert_embeddings.cpu(), masks=attention_mask.cpu(), epoch=epoch, batch_no=batch_count, kl_coefficent=kl_coefficent)
                ...
                visualize_embedding(bert_embedding=x_hat.detach().cpu(), masks=attention_mask.detach().cpu(), epoch=epoch, batch_no=batch_count, kl_coefficent=kl_coefficent, type="decoder_output")
                visualize_latent_vector_space(latent_vector_space=z.detach().cpu(), epoch=epoch, batch_count=batch_count, kl_coefficent=kl_coefficent)

                # print(f"Generated Sentence: {bert_tokenizer.decode(x_hat.argmax(dim=-1)[0], skip_special_tokens=True)}")

                loss = calculate_vae_loss(x, x_hat, mu, logvar, kl_coefficent)
